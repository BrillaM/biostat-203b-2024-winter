...
labevents_tble <- tbl(con_bq, "labevents") |>
  ...
  filter(item %in% c(50912, 50971, 50983, 50902, 50882, 51221, 51301, 50931)) |>
...
```{r}
# # TODO
 mimic_icu_cohort <- tbl(con_bq, "icustays")
  left_join(admissions_tble, by = c("subject_id") |>
  left_join(patients_tble, by = c("subject_id")) |>
    mutate(age_subject = anchor_age + year(admittime) - anchor_year) |>
    filter(age_subject >= 18) |>
...
# Define UI
ui <- fluidPage(
  titlePanel("ICU Cohort Data Explorer"),
  tabsetPanel(
    tabPanel("Summary", 
             selectInput("variable", "Choose a variable:", choices = c("Demographics", "Lab Measurements", "Vitals")),
             plotOutput("summaryPlot")),
    tabPanel("Patient Details",
             textInput("patientID", "Enter Patient ID:"),
             dataTableOutput("patientADT"),
             dataTableOutput("patientICU"))
  ))


# Define server logic
server <- function(input, output) {
  # Assume data is loaded and prepared here
  
  output$summaryPlot <- renderPlot({
    # Placeholder for variable-based plotting logic
  })
  
  output$patientADT <- renderDataTable({
    # Placeholder for patient-specific ADT details
  })
  
  output$patientICU <- renderDataTable({
    # Placeholder for patient-specific ICU stay details
  })
}

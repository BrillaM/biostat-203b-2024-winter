...
The possible value for `insurance` is 
, and the possible value for `ethnicity` is :
There is 9 unique values for `admission_type` .
There is 11 unique values for `admission_location` .
There is 3 unique values for `insurance` .
There is 33 unique values for `ethnicity` .
...
**answer**: `cal` displays the current month's calendar.

...
**Answer** :The data files are distributed as `.csv.gz` files because they are compressed. The data files are big and it is more efficient to store them in compressed format.
```{bash}
ls -l ~/mimic/hosp
ls -l ~/mimic/icu
```
...
**Answer** :
`zcat` is used to display the contents of a compressed file. `zless` and `zmore` are used to display the contents of a compressed file one page at a time. `zgrep` is used to search for a pattern in a compressed file.
...
**Answer**
admissions has 431232 lines, labevents has 118171368 lines, and patients has 299713 lines.
```{bash}
for datafile in ~/mimic/hosp/{a,l,pa}*.gz
do
  zcat < $datafile | wc -l
...
**Answer**:There are 431232 rows in the `admissions.csv.gz` file. There are 431232 unique patients in the `admissions.csv.gz` file. There are 299713 patients in the `patients.csv.gz` file. It doesn't match the number of patients in the `patients.csv.gz` file.
```{bash}
zcat < ~/mimic/hosp/admissions.csv.gz | head -n 5
zcat < ~/mimic/hosp/admissions.csv.gz | wc -l
zcat < ~/mimic/hosp/admissions.csv.gz | awk  '{print $1}' | sort -u | wc -l
...
**Answer**: The possible values taken by each of the variable `admission_type`, `admission_location`, `insurance`, and `ethnicity` are as follows:
```{bash}
zcat < ~/mimic/hosp/admissions.csv.gz | awk -F, '{print $9}' | sort | uniq -c
zcat < ~/mimic/hosp/admissions.csv.gz | awk -F, '{print $10}' | sort | uniq -c
zcat < ~/mimic/hosp/admissions.csv.gz | awk -F, '{print $11}' | sort | uniq -c
zcat < ~/mimic/hosp/admissions.csv.gz | awk -F, '{print $12}' | sort | uniq -c
```

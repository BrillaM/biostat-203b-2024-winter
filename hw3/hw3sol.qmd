...
```
```{r}
sid_dia_fianl <- inner_join(diag_icd_tbl, d_icd_diagnoses_tbl, 
                            by = c("icd_code","icd_version")) |>

count(sid_dia_fianl, long_title, sort = True) |> 
```
  
```{r}
sid_proc_final <- inner_join(procedures_icd_tbl,d_icd_procedures_tbl, 
                             by = c("icd_code","icd_version")) |>
...
sid_adt |>
  ...
  ggplot ()+
  geom_point(data = sid_proc_final, aes(x = chartdate, y = seq_num, 
                                        color = long_title, shape = long_title))
+
...
ggplot(patients_tble, aes(x=gender, y=anchor_age)) +
  ...
  theme_minimal()
```
**answer:** We can see Male have a wider range on anchor age 50-70 than female. Female has a wider range on anchor age 20-40 than male, also female has more older patient than male. 

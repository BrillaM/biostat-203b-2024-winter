...
```
**answer**: `fread()` is the fastest, taking 0.420 seconds and uses the least memory at 50.13 MB. The other two functions, read.csv() and read_csv(), are slower and consume more memory, with no differences in default data types parsed.
...
```
**answer**: The first 10 lines of the new file `labevents_filtered.csv.gz` are displayed. There are 2889731 lines in this new file. The system run time is 0.180 seconds.
...
```{r}
labevents <- read_csv_arrow("labevents.csv")
labevents %>%
